...
void	ft_count_nodes_ast(t_ast *tree, int *len)
{
	if (!tree)
		return ;
	(*len)++;
	ft_count_nodes_ast(tree->left, len);
	ft_count_nodes_ast(tree->right, len);
}

void	ft_print_count_nodes_ast(t_ast *tree)
{
	int	len_ast;

	len_ast = 0;
	ft_count_nodes_ast(tree, &len_ast);
	printf("%d\n", len_ast);
}
